...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    from scheme_forms import SPECIAL_FORMS
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        return SPECIAL_FORMS[first](rest, env)
    else:
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        operater = scheme_eval(first, env)
        arguments = map_link(lambda x: scheme_eval(x, env), rest)
        return scheme_apply(operater, arguments, env)
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        while current is not nil:
            temp.append(current.first)
            current = current.rest

        args = temp  # now args is a Python list
